...
async def process_choice1(callback_query: types.CallbackQuery):
    ...
    button3 = types.InlineKeyboardButton(
        text="Получить свою статистику", callback_data="choice1"
    )
    button4 = types.InlineKeyboardButton(
        text="Получить статистику пользователя", callback_data="choice2"
    )
    keyboard.add(button3, button4)
    await bot.send_message(
        callback_query.from_user.id, "Выберите одну из", reply_markup=keyboard
    )
